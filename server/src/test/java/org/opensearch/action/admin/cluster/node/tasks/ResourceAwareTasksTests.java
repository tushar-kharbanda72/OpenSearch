...
public class ResourceAwareTasksTests extends TaskManagerTestCase {
    ...
    public void testOnDemandRefreshWhileFetchingTasks() throws InterruptedException {
        ...
        taskTestContext.operationStartValidator = threadId -> {
            ...
            assertTrue(taskInfo.getResourceStats().getResourceUsageInfo().get("total_resource_consumption").getCpuTimeInNanos() > 0);
            assertTrue(taskInfo.getResourceStats().getResourceUsageInfo().get("total_resource_consumption").getMemoryInBytes() > 0);
        };
